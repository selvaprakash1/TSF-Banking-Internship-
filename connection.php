...
$username ="if0_36923335";
$password = "Sivaparvathi1";
$databse ="if0_36923335_bank";
...
function check_name($from,$to,$amount){
    ...
    // database parameters
     $user       = 'if0_36923335';
     $password   = 'Sivaparvathi1';
     $dns ='mysql:host=sql311.infinityfree.com;dbname=if0_36923335_bank';
    ...
    try {
        ...
        $sql = 'SELECT amount FROM customers WHERE Cust_Name=:from';
        $stmt = $db->prepare($sql);
        ...
        $st1 = $db->prepare(
            ...
            `Amount`= Amount - :amt where Cust_Name = :fro "
        );
        $st2 = $db->prepare(
            "update customers set
            `Amount`= Amount + :amt where Cust_Name = :to "
        );
